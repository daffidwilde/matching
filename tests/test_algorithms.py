...
class TestGaleShapleyAlgorithms(unittest.TestCase):
    ...
    def test_galeshapley(self):
        ...
        suitor_preferences = {'A': ['D', 'E', 'F'],
                              'B': ['D', 'F', 'E'],
                              'C': ['F', 'D', 'E']}

        reviewer_preferences = {'D': ['B', 'C', 'A'],
                                'E': ['A', 'C', 'B'],
                                'F': ['C', 'B', 'A']}
    ...
    def test_extended_galeshapley(self):
        ...
        suitor_preferences = {'A': ['C'],
                              'S': ['C', 'M'],
                              'J': ['C', 'G', 'M'],
                              'L': ['M', 'C', 'G'],
                              'D': ['C', 'M', 'G']}

        reviewer_preferences = {'M': ['D', 'J'],
                                'C': ['D', 'A', 'S', 'L', 'J'],
                                'G': ['D', 'A', 'J', 'L']}

        capacities = {'M': 2, 'C': 2, 'G': 2}

        str_matching, rvwr_matching = extended_galeshapley(suitor_preferences,
                                                           reviewer_preferences,
                                                           capacities)

        self.assertEqual(
            set(suitor_preferences.keys()),
            set(str_matching.keys())
        )
        self.assertEqual(
            set(reviewer_preferences.keys()),
            set(rvwr_matching.keys())
        )

        expected_str_matching = {'A': 'C', 'S': None,
                                 'J': 'G', 'L': 'G', 'D': 'C'}
